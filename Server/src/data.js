...
function handleUserCommand(connectionInformation, data) {
  ...
  switch (command) {
    ...
    case "PASV":
      ...
      const serverAddress = '127.0.0.1'.replace(/\./g, ',');
      let port1 = Math.floor(22222/ 256);
      let port2 = Math.floor(22222 % 256);
      const response = `227 (${serverAddress},${port1},${port2}).\r\n`;
      srv.listen(22222, 'localhost', () => console.log('Server FTP launched on port 22222'));
